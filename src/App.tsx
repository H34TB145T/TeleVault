...
function foldersAt(files: VaultFile[], storedFolders: VaultFolderRecord[], currentPath: string, includeStoredFolders: boolean): VaultFolder[] {
  ...
  if (includeStoredFolders) {
    for (const folder of storedFolders) {
      ...
      const existing = folders.get(childPath) ?? { path: childPath, name, fileCount: 0, size: 0, accountName: "TeleVault", latestCreatedAt: folder.createdAt };
  ...
  for (const file of files) {
    ...
    if (existing.accountName === "TeleVault") existing.accountName = file.accountName;
...
function Skeleton() {
  return <div className="loading-screen"><img src="/assets/televault-logo.png" /><div className="loader" /><p>Opening your vault…</p></div>;
}
...
function CreateFolderModal({ parentPath, onClose, onCreate }: { parentPath: string; onClose: () => void; onCreate: (name: string) => Promise<void> }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <form className="modal create-folder-modal" onSubmit={submit}>
      ...
      <div className="create-folder-destination"><Folder size={18} /><span><small>LOCATION</small><strong>{parentPath || "My TeleVault"}</strong></span></div>
...
function EmptyState({ category, onUpload }: { category: Category; onUpload: () => void }) {
  return (
    <div className="empty-state">
      ...
      <p>Drop files anywhere or choose them from your device. TeleVault handles chunking automatically.</p>
  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose}>
      <div className="modal upload-modal" onMouseDown={(e) => e.stopPropagation()}>
        <div className="modal-head"><div><span className="eyebrow">NEW TRANSFER</span><h2>Upload to TeleVault</h2></div><button className="icon-button" onClick={onClose}><X /></button></div>
        ...
        <div className="option-card">
          <div className="option-icon"><Copy /></div><div><strong>Skip exact duplicates</strong><p>When a same-size candidate exists, TeleVault hashes it locally and skips the upload only when the content is identical.</p></div>
...
function AccountModal({ account, onClose, onConnected }: { account?: Account; onClose: () => void; onConnected: () => void }) {
  ...
  return (
    <div className="modal-backdrop" onMouseDown={onClose}><div className="modal account-modal" onMouseDown={(e) => e.stopPropagation()}>
      ...
      {step === "details" && <><p className="modal-copy">{account ? "TeleVault will reuse this vault’s protected phone number and Telegram API credentials. Reconnecting keeps every existing file attached to the same account." : "Use your own Telegram application credentials. TeleVault stores the session only on this device and never asks for your Telegram password outside this sign-in window."}</p>
        ...
        <div className="notice"><ShieldCheck size={17} /> Credentials remain in TeleVault’s protected local application data.</div>
      ...
      {step === "done" && <div className="auth-step"><div className="success-orb"><Check /></div><h3>Account connected</h3><p>TeleVault can now store files in this account’s Saved Messages.</p><button className="button primary wide" onClick={onClose}>Open vault</button></div>}
    </div></div>
  );
}
...
function ShareModal({ file, folder, folderFiles = [], confirmAction, onClose, onQueued }: { file?: VaultFile; folder?: VaultFolder; folderFiles?: VaultFile[]; confirmAction: ConfirmAction; onClose: () => void; onQueued: () => void }) {
  ...
  const name = folder?.name ?? file?.name ?? "TeleVault item";
  ...
  const send = async () => {
    ...
    const encryptionWarning = encrypted
      ? `TeleVault will reconstruct encrypted content locally and send ${isFolder ? "the files" : "this file"} as normal readable ${isFolder ? "documents" : "a document"}, one at a time. A regular Telegram cloud chat is not end-to-end encrypted.`
      : `TeleVault will upload ${isFolder ? `${itemCount} individual files` : "a normal readable copy"}. The recipient can save or forward them, and deleting your vault copy will not remove theirs.`;
  ...
  return <div className="modal-backdrop share-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !sending) onClose(); }}>
    <section className="modal share-modal" role="dialog" aria-modal="true" aria-labelledby="share-title">
      ...
      {encrypted && <div className="notice warning"><LockKeyhole size={17} /><span><strong>Readable copy warning</strong><br />The recipient cannot use TeleVault’s encrypted chunks. Sending will create readable Telegram {isFolder ? "copies one at a time" : "a copy"} without revealing your vault recovery key.</span></div>}
...
function FileActionModal({ file, action, folders, onClose, onComplete }: { file: VaultFile; action: FileAction; folders: VaultFolderRecord[]; onClose: () => void; onComplete: (file: VaultFile) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}><form className="modal file-action-modal" onSubmit={submit}><div className="modal-head"><div><span className="eyebrow">FILE MANAGEMENT</span><h2>{action === "rename" ? "Rename file" : action === "move" ? "Move file" : "Copy file"}</h2></div><button type="button" className="icon-button" disabled={busy} onClick={onClose}><X /></button></div>
    <div className="share-file-summary"><span className="small-file-icon"><FileText /></span><span><strong>{file.name}</strong><small>{formatBytes(file.size)} · {file.folderPath || "My TeleVault"}</small></span></div>
    ...
    {action !== "rename" && <label className="field"><span>Destination folder</span><select value={folderPath} onChange={(event) => setFolderPath(event.target.value)}><option value="">My TeleVault</option>{folders.map((folder) => <option key={folder.id} value={folder.path}>{folder.path}</option>)}</select></label>}
...
function LockScreen({ onUnlock }: { onUnlock: (status: LockStatus) => void }) {
  ...
  return <div className="lock-screen"><form className="lock-card" onSubmit={submit}><img src="/assets/televault-logo.png" alt="TeleVault" /><div className="lock-orb"><Lock /></div><span className="eyebrow">APP LOCK</span><h1>TeleVault is locked</h1><p>Your vault catalogue and local preview endpoints remain inaccessible until you unlock the app.</p><label className="field"><span>Password</span><input autoFocus type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" /></label>{error && <div className="notice danger"><CircleAlert size={16} /> {error}</div>}<button className="button primary wide" disabled={!password || busy}>{busy ? <RefreshCw className="spin" /> : <LockKeyhole />} Unlock TeleVault</button></form></div>;
}
...
function SettingsView({ data, onRefresh, onCacheCleared, setTheme, onLock, confirmAction }: { data: Dashboard; onRefresh: () => void; onCacheCleared: () => void; setTheme: (theme: "light" | "dark" | "system") => void; onLock: (status: LockStatus) => void; confirmAction: ConfirmAction }) {
  ...
  const disableLock = async () => {
    if (!await confirmAction({ title: "Remove app lock?", message: "TeleVault will stop asking for this password at startup and after inactivity. Your vault encryption and operating-system keychain remain unchanged.", confirmLabel: "Remove app lock", tone: "warning" })) return;
    setLockBusy(true); setLockError("");
  ...
  const recover = async () => {
    ...
    if (!await confirmAction({ title: "Rebuild this vault from Telegram?", message: "TeleVault will scan Saved Messages for signed TeleVault manifests and restore missing catalogue entries locally. It will not upload, forward or delete Telegram messages.", confirmLabel: "Scan and restore", tone: "warning" })) return;
  ...
  const changeNotifications = async (enabled: boolean) => {
    if (enabled && !await api.requestNotificationPermission()) {
      alert("Notifications are disabled in macOS settings. Allow TeleVault notifications there, then try again.");
      return;
    }
  ...
  const checkForUpdate = async () => {
    ...
    if (!await confirmAction({ title: "Check for a signed update?", message: "TeleVault will contact the configured HTTPS release endpoint. If an update exists, its signature will be verified before installation and the app will restart.", confirmLabel: "Check for update", tone: "warning" })) return;
    setUpdateBusy(true); setUpdateMessage("");
    try {
      const result = await api.installAvailableUpdate();
      if (result === "current") setUpdateMessage("TeleVault is already up to date.");
  ...
  return <div className="settings-page"><div className="page-title"><div><span className="eyebrow">PREFERENCES</span><h1>Settings</h1><p>Control privacy, recovery, performance, caching and the web companion.</p></div></div>
    ...
    <section className="settings-section"><div className="section-title"><ShieldCheck /><div><h3>Signed automatic updates</h3><p>Every public update must be cryptographically signed before TeleVault will install it.</p></div></div><div className="settings-card"><div className="setting-row"><span><strong>{data.automaticUpdatesConfigured ? "Release channel configured" : "Waiting for release signing setup"}</strong><small>{data.automaticUpdatesConfigured ? "This build contains a release endpoint and verification public key." : "No updater endpoint or public verification key is embedded in this build. Automatic checks stay disabled safely."}</small></span>{data.automaticUpdatesConfigured ? <Check className="success" /> : <CircleAlert className="warning" />}</div>{data.automaticUpdatesConfigured && <button className="setting-row action" disabled={updateBusy} onClick={checkForUpdate}><span><strong>{updateBusy ? "Checking and verifying…" : "Check for updates"}</strong><small>Only an artifact signed by the configured TeleVault release key can be installed.</small></span>{updateBusy ? <RefreshCw className="spin" /> : <RefreshCw />}</button>}{updateMessage && <div className="notice"><Info size={16} /> {updateMessage}</div>}</div></section>
    ...
    <section className="settings-section"><div className="section-title"><Cloud /><div><h3>Web companion</h3><p>Available only while the TeleVault desktop app is running.</p></div></div><div className="settings-card"><button className="setting-row action" onClick={async () => { if ("__TAURI_INTERNALS__" in window) { const { openUrl } = await import("@tauri-apps/plugin-opener"); await openUrl("http://127.0.0.1:7468"); } else { window.location.href = "http://127.0.0.1:7468"; } }}><span><strong>Open web interface</strong><small>http://127.0.0.1:7468 · loopback only</small></span><Link2 /></button></div></section>
  </div>;
}

function AboutView() {
  return <div className="content-page about-page"><div className="about-card"><div className="about-brand"><img src="/assets/televault-logo.png" alt="TeleVault logo" /><div><span className="eyebrow">ABOUT TELEVAULT</span><h1>TeleVault</h1><p>Your private, encrypted file vault backed by Telegram Saved Messages.</p></div></div><div className="about-details"><div><span>Version</span><strong>0.1.0</strong></div><div><span>Developer</span><strong>H34TB145T</strong></div><div><span>Storage</span><strong>Telegram Saved Messages</strong></div><div><span>Privacy</span><strong>Optional client-side encryption</strong></div></div><div className="about-credit"><ShieldCheck size={18} /><span><small>DESIGNED AND DEVELOPED BY</small><strong>H34TB145T</strong></span></div></div></div>;
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    for (const transfer of data.transfers) {
      ...
      const notification = transfer.state === "complete" ? ["Transfer complete", "A TeleVault transfer completed successfully."] : transfer.state === "failed" ? ["Transfer failed", "A TeleVault transfer needs your attention."] : transfer.state === "paused" ? ["Transfer paused", "A TeleVault transfer is paused."] : transfer.state === "waiting" ? ["Telegram asked TeleVault to wait", "A transfer will retry automatically after Telegram's wait period."] : null;
  ...
  useEffect(() => {
    ...
    void api.availableUpdateVersion().then((version) => {
      if (version) void api.sendNotification("Signed TeleVault update available", `Version ${version} is ready to review in Settings.`);
  ...
  return <div className={`app-shell ${selected ? "with-preview" : ""}`}>
    <aside className={`sidebar ${mobileNav ? "open" : ""}`}>
      <div className="brand"><img src="/assets/televault-logo.png" /><span>TeleVault</span></div>
      <nav>
    ...
    {dragging && <div className="drop-overlay"><div><Upload size={42} /><h2>Drop to upload</h2><p>TeleVault will categorize, encrypt and chunk automatically.</p></div></div>}
